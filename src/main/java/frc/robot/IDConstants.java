package frc.robot;

public final class IDConstants 
{
    public static final int pigeonID = 53;

    public static final class Climber 
    {
        public static final int mLeftClimbID = 13;
        public static final int mRightClimbID = 14;

        public static final int leftSwitchID = 5;
        public static final int rightSwitchID = 6;
        
        public static final int mStabiliserID = 15;
        public static final int mBuddyClimbID = 16;
    }

    public static final class Intooter 
    {
        public static final class Shooter 
        {
            public static final int mTopShooterID = 17;
            public static final int mBottomShooterID = 18;
        }

        public static final class Intake
        {
            public static final int mIntakeID = 19;
            public static final int mIndexerID = 20;

            public static final int beamBreakID = 9;
            public static final int stabiliserLimitID = 4;
        }

        public static final class Pivot
        {
            public static final int mLeftPivotID = 21;
            public static final int mRightPivotID = 22;

            public static final int leftOutSwitchID = 3;
            public static final int leftInSwitchID = 0;
            public static final int rightInSwitchID = 2;
            public static final int rightOutSwitchID = 1;
        }
    }
}
