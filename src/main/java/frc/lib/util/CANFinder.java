...
public class CANFinder {
    ...
    private static CANFinder myInstance;
    public static CANFinder getFinder()
    {
        if (myInstance == null)
        {
            myInstance = new CANFinder();
        }
        return myInstance;
    }
    ...
    private final static int maxCanID = 63;
	...
	private ArrayList<String> Find() {
		...
		long[] pcm_timeStamp0 = new long[maxCanID];
		long[] srx_timeStamp0 = new long[maxCanID];
		
		pdp0_timeStamp0 = checkMessage(0x08041400,0);
		for(int i=0;i<maxCanID;++i) {
		...
		long[] pcm_timeStamp1 = new long[maxCanID];
		long[] srx_timeStamp1 = new long[maxCanID];
